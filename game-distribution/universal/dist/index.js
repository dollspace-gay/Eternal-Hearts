...
var insertGameStateSchema = createInsertSchema(gameStates).pick({
  userId: true,
  slotName: true,
  gameData: true
});
...
async function registerRoutes(app2) {
  app2.get("/api/game-states", async (req, res) => {
    try {
      const userId = 1;
      const gameStates2 = await storage.getGameStates(userId);
      res.json(gameStates2);
    } catch (error) {
      res.status(500).json({ message: "Failed to get game states" });
    }
  });
  app2.post("/api/game-states", async (req, res) => {
    try {
      const userId = 1;
  ...
  app2.delete("/api/game-states/:id", async (req, res) => {
    try {
      const userId = 1;
